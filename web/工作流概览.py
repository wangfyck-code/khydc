...
if insight is None:
    st.info(
        ...
        "录入概念 + 现款 baseline → 生成 skill 调用提示词 → 拷贝到 Claude Code 跑。"
    )
    st.stop()
...
if status == "pending":
    st.warning(
        f"⏳ 当前 run `{slug}` 是『**待生成**』状态——已录入 input.json，还没跑 skill。"
        "请到『立项概念输入 → 当前 run baseline』复制调用提示词到 Claude Code。"
    )
...
| 步骤 | 页面 | 你做什么 |
|---|---|---|
| **1** | **立项概念输入** | 录入升级概念 + 现款 baseline → 生成 skill 调用提示词 |
| **1.5** | _（Claude Code 中）_ | 拷贝提示词跑 skill → 产出 `insight.json` 落到 `runs/<slug>/` |
| **2** | **跨行业洞察地图** | 看对标行业全景：十维 / 演化时间线 / 可参考借鉴 |
| **3** | **机会矩阵与产品概念** | 看气泡图 + 卡片 → 给每条机会打 PM 判断 / 备注 / 覆盖打分 / 🧺 加入篮 |
| **4** | **立项候选篮** | 一键导出立项一页纸 `onepager_pm.md`（PM 决策 + 借鉴来源 + 验证清单） |
| **5** | **验证执行清单** | 上市前要验证的假设清单（按行业组织） |
"""
)
st.caption(
    "所有 PM 标注实时落盘到 `runs/<slug>/decisions.json`；下次打开还在。"
    "切 run 后侧栏会同步当前 run 的立项候选篮计数。"
)
